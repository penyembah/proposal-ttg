...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth]{dashboard-light}
  \caption{Tampilan dasbor Welya: sapaan personal, jadwal hari ini yang
    tersinkron dari Google Calendar, deadline terdekat hasil ekstraksi
  otomatis dari Gmail, serta rekomendasi prioritas dari AI}
  \label{gbr:dasbor}
\end{figure}
...
\begin{figure}[htbp]
  ...
  \caption{Tampilan workspace kolaborasi untuk kerja kelompok beserta
  progres tim dan pembagian tugas anggota}
  \label{gbr:workspace}
\end{figure}
...
\begin{table}[htbp]
  ...
  \begin{tabular}{@{}lcccc@{}}
    ...
    \midrule
    Analisis kebutuhan  & $\checkmark$ &              &              &              \\
    Perancangan sistem  & $\checkmark$ & $\checkmark$ &              &              \\
    Implementasi        &              & $\checkmark$ & $\checkmark$ &              \\
    Pengujian           &              &              & $\checkmark$ & $\checkmark$ \\
    Evaluasi \& laporan &              &              &              & $\checkmark$ \\
    Artikel publikasi \& HKI &         &              &              & $\checkmark$ \\
    \bottomrule
  \end{tabular}
\end{table}
